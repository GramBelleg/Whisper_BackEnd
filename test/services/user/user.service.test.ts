...
describe('test change last seen privacy query', () => {
    ...
    it('should throw error if user not found', async () => {
        const privacy = 'Nobody';
        await expect(userServices.changeLastSeenPrivacy(0, privacy)).rejects.toThrow(new HttpError('User not found', 404));
    });
});
...
describe('test change pfp privacy query', () => {
    ...
    it('should throw error if user not found', async () => {
        const privacy = 'Nobody';
        await expect(userServices.changePfpPrivacy(0, privacy)).rejects.toThrow(new HttpError('User not found', 404));
    });
}); 
...
describe('test change story privacy query', () => {
    ...
    it('should throw error if user not found', async () => {
        const privacy = 'Contacts';
        await expect(userServices.changeStoryPrivacy(0, privacy)).rejects.toThrow(new HttpError('User not found', 404));
    });
});