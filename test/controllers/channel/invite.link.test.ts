...
describe("Invite Link", () => {
    ...
    it("should throw error invalid token", async () => {
        ...
        expect(response.body.message).toBe("Invalid invite link");
    });
    it("should throw error failed to fetch clients", async () => {
        const participants = [1, 2, 3];
        const userChat = {};
        (joinChannel as jest.Mock).mockResolvedValue({
            participants,
        ...
        expect(response.body.message).toBe("Failed to retrieve clients");
    });
});
