...
export const setupStoryEvents = (socket: Socket, connectedUserId: number, clients: Map<number, Socket>) => {
    socket.on(
        "story",
    ...
    socket.on(
        "likeStory",
        socketWrapper(async (userId: number ,storyId: number, userName: string, profilePic: string) => {
            try{
                await storyController.likeStory(connectedUserId, storyId);
                await storyHandler.likeStory(userId, clients, "likeStory", {userId:connectedUserId, storyId:storyId, userName:userName, profilePic:profilePic});
            }
            catch (e: any) {
    ...
    socket.on(
        ...
        socketWrapper(async (userId: number ,storyId: number, userName: string, profilePic: string) => {
            try {
                ...
                await storyHandler.viewStory(userId, clients, "viewStory", {userId:connectedUserId, storyId:storyId, userName:userName, profilePic:profilePic});
            } catch (e: any) {
